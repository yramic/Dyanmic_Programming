        ...
        for i_z in range(Constants.D):
            for i_y in range(Constants.N):
                for i_x in range(Constants.M):
                    i = i_x + i_y * y_step + i_z * z_step + i_t * g_t_step
                    t_step = g_t_step if i_t != Constants.T - 1 else g_t_step_wrap
                    x_step = g_x_step if i_x != Constants.M - 1 else g_x_step_wrap
                    x_step_back = -g_x_step if i_x != 0 else -g_x_step_wrap
                    ...
                    if i_t != Constants.T - 1:
                        ...
                        if i_y == Constants.N - 1:
                            P[i][i + t_step][Constants.V_STAY] = Constants.ALPHA * (
